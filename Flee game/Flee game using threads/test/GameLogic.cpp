...
//	glutTimerFunc(100, statusBarAknowledge, 0); blocking
...
void sustainingViewUpdate(int params) {
	...
	//glutTimerFunc(10, sustainingViewUpdate, 0);

}

int gameLogicClock = 0;
void sustainingGameLogicUpdate(int redudent)
{
	bool Continue;
	while (1){ //keep repeating and repeating and repeating........

		velocity = 1.3 - exp(-8 * (TimeRemainingInMSC / totalTime));
	...
	if (!(ghostModeOn || pcModeOn || someoneWon))
	{
		for (int i = 0; i < gemsCount; i++)
			if ((GemsLocations[i].row == playercell.row) &&( GemsLocations[i].column == playercell.column))
			{
				 ...
				 deltaMove = .1;
				 deltaAngleDueTokeyboard = -.002f;
				 lookingUPAngle -= .05;
				 deltaHeight = .005f;	
				 Continue = true; break;

				 gameLogicClock = gameLogicClock - 5;
	...
	if (gameLogicClock == 0 && !GAMEOVER) { //randomly pick a path for ghosts every 1 second (ghost next move)
		...
		for (int i = 0; i < ghostCount; i++) {
			...
			if (pcModeOn) {
				...
				if (1 == winnersCount) {
					winnerIndex = i;
					//glutTimerFunc(5, sustainingGameLogicUpdate, params - 5);
					//return;
...
//	glutTimerFunc(5, sustainingGameLogicUpdate, params - 5);//repeat

}


void smoothDecrease(int TimeRemaining) {

	if (TimeRemaining > 0) {
		cameraHeight -= (heightMem - 2) / (20);
		glutTimerFunc(10, smoothDecrease, TimeRemaining - 10);
	}
