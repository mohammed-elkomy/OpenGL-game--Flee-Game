...
ghostControlParameters.setParameters();//ghost step for each clock cycle
	...
	//glutTimerFunc(100, sustainingGameLogicUpdate, 0); this causes flickering due to block !!! <<<<< important performance tip
	//Sleep(100);//keep montoring the game logic (winners , losers and ghosts AI) 
	std::thread gameLogic(sustainingGameLogicUpdate,0);


	//glutTimerFunc(100, sustainingViewUpdate, 0);  this causes flickering due to block !!!<< << < important performance tip
	//Sleep(100);//update the view 
	std::thread viewThread(sustainingViewUpdate, 0);
	//glutTimerFunc(100, sustainingViewUpdate, 0);	//update the view

	//glutTimerFunc(100, statusBarAknowledge, 0)
//	Sleep(100);//keep track of game mode
	std::thread statusBarThread(statusBarAknowledge, 0);
